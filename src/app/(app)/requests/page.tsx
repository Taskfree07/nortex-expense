    ...
    <>
      ...
      {requests.length === 0 ? (
        <EmptyState
          ...
          action={<LinkButton href="/requests/new" variant="primary">Raise a travel request</LinkButton>}
        />
      ) : (
        <Panel>
            ...
            <tbody>
              {requests.map((request) => {
                ...
                return (
                  <tr key={request.id}>
                    <td>
                      <Link href={`/requests/${request.trqId}`} className="ident text-stamp hover:underline">
                        {request.trqId}
                      </Link>
                    ...
                    <td className="amount">
                      {request.advanceDisbursed > 0 ? (
                        ...
                        <span className="text-ink-faint">{formatINR(request.advanceRequested)} asked</span>
                      ) : (
                    ...
                    <td>
                      {claim ? (
                        ...
                        <Link href={`/requests/${request.trqId}`} className="text-xs text-stamp hover:underline">
                          Not filed yet
                        </Link>
                      ) : (
