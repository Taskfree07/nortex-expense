...
for (const [a, b] of ranges)
  for (let r = a; r <= b; r++) {
    const who = s.getCell(`G${r}`).value,
      amt = Number(s.getCell(`H${r}`).value) || 0;
    if (who === "Employee") emp += amt;
    else if (who === "Company") comp += amt;
  }
const round2 = (n) => Math.round(n * 100) / 100;
const disallowed = Number(s.getCell("H46").value),
  advance = Number(s.getCell("H48").value);
...
console.log(
  net === 26388.44 && round2(net - advance) === 6388.44 ? "MATCHES the hand-worked settlement" : "MISMATCH",
);
